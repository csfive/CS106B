...
/* Border colors for each disk.  These are just darker versions of the disk
 * colors given above.
 */
const string kDiskBorderColors[] = {"#008b8b", "#013220", "#ffff00", "#ff8c00",
                                    "#8b0000", "#00008b", "#a9a9a9", "#e75480"};
...
void initHanoiDisplay(int numDisks) {
    ...
    /* Determine the height of a single disk.  We want to size the disks such
     * that each spindle can have all the disks on top of it, then some vertical
     * space to show the top of each spindle, then two blank spaces above it.
     * This works out to needing a number of "virtual disks" equal to the total
     * number of disks actually used, plus four extras.
     */
...
void setupSpindles() {
    ...
    for (int i = 0; i < kNumSpindles; i++) {
         ...
         * one (so the spindle is still visible).  It will also be
         * bottom-aligned.
         */
        double height = (hg.numDisks + 1) * hg.diskHeight;
        double y = hg.window->getHeight() - height;

        /* Determine where the center line is, then center the rectangle around
         * that. */
        spindle.centerX = (spindle.startX + spindle.endX) / 2.0;
        spindle.rect = new GRect(spindle.centerX - kSpindleWidth / 2.0, y,
                                 kSpindleWidth, height);
...
void setupDisks() {
    for (int i = 0; i < hg.numDisks; i++) {
         ...
         * Therefore:
         *
         * width = ((kMinDiskWidth - workspaceWidth) / (numDisks - 1)) * i +
         * workspaceWidth
         *
         * There is an edge case here when numDisks = 1, so we will special-case
         * it.
         */
        ...
        if (hg.numDisks == 1) {
            ...
            width = ((kMinDiskWidth - workspaceWidth) / (hg.numDisks - 1)) * i +
                    workspaceWidth;
        }

        /* Given the width, the x coordinate can be found by taking the center
         * line of the spindle and backing off by half the width.
         */
        double x = hg.spindles[0].centerX - width / 2.0;

        /* We can determine the y coordinate of the disk by using our existing
           function
...
double diskYPosition(int spindle) {
     ...
     */
    return hg.window->getHeight() -
           (hg.spindles[spindle].disksHere.size() + 1) * hg.diskHeight;
}
 ...
 * and 1 (end), interpolates where along the trajectory the disk would be at
 * that time using a cubic Hermitian spline.  This makes the animation look
 * significantly smoother than before.
 *
 * http://en.wikipedia.org/wiki/Cubic_Hermite_spline
 */
double interpolate(double t) { return -2 * t * t * t + 3 * t * t; }
...
void animateDiskPath(GRect *disk, double endX, double endY, double totalTime) {
    ...
    for (int i = 0; i < kNumAnimationSteps; i++) {
        ...
        double x =
            startX +
            (endX - startX) * interpolate(double(i) / (kNumAnimationSteps - 1));
        double y =
            startY +
            (endY - startY) * interpolate(double(i) / (kNumAnimationSteps - 1));
        disk->setLocation(x, y);

        /* Pause to let the animation progress.  This will not work out to
         * pausing for exactly the right amount of time because there's some
         * latency involved in the graphics calls, but it's "close enough."
         */
...
void moveSingleDisk(char startCh, char finishCh) {
    ...
    /* Determine how long the animation must take for each of the three parts of
     * the animation.
     */
