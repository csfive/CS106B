...
void findSolution(int n, char source, char dest, char aux) {
    ...
    if (n == 1) {
        ...
        findSolution(n-1, source, aux, dest); // move all the other disks to the aux pin
        moveSingleDisk(source, dest); // move largest disk
        findSolution(n-1, aux, dest, source); // move all other disks from aux to the dest pin
    }
}
